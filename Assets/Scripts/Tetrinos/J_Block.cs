...
public class J_Block : Tetrino
{
    ...
    public override void populateSegments()
    {
        ...
        newSegment = Instantiate(mBaseSegment, new Vector3(BaseBlock.BLOCK_SIZE + X_OFFSET, 0, -1), Quaternion.identity) as GameObject;
        newSegment.transform.SetParent(transform, false);
        mSegments.Add(newSegment);

        newSegment = Instantiate(mBaseSegment, new Vector3((BaseBlock.BLOCK_SIZE * 2) + X_OFFSET, 0, -1), Quaternion.identity) as GameObject;
        newSegment.transform.SetParent(transform, false);
        mSegments.Add(newSegment);
    }
    ...
    public override bool insertIntoModel(Vector3 initialPosition)
    {
        ...
        if (!mGameBoardModel.setModel(mSegments[2], baseX + 1, baseY))
        {
            return false;
        }

        if (!mGameBoardModel.setModel(mSegments[3], baseX + 2, baseY))
        {
            return false;
        }
    ...
    {
        transform.localPosition += new Vector3(80f, 130f, 0);
    }
}
